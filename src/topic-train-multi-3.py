...
from vdcnn_multi_3 import MultiVDCNN
...
n_out_1 = train_soft20.shape[1]
n_out_2 = train_soft.shape[1]
n_out_3 = train_soft200.shape[1]
...
print('MultiVDCNN setting: emb_dim={} n_out_1={}, n_out_2={}, n_out_3={}, depth={}'.format(len(char2id)+1, n_out_1, n_out_2, n_out_3, sum(depth)*2+1))

gpu_id = args.gpu

model = MultiVDCNN(len(char2id)+1, n_out_1, n_out_2, n_out_3, depth)
if gpu_id >= 0:
    model.to_gpu(gpu_id)


train_y = np.concatenate([train_y.reshape(-1, 1), train_soft20, train_soft, train_soft200], axis=1).astype(np.float32)
test_y = np.concatenate([test_y.reshape(-1, 1), test_soft20, test_soft, test_soft200], axis=1).astype(np.float32)
...
result_dir = '../results/multi3_softtopic_{}_{}_{}_{}_depth{}_valid{}'.format(mode, n_out_1, n_out_2, n_out_3, sum(depth)*2+1, args.valid)
