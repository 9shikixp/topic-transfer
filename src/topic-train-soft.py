...
def lossfun(x, t):
    with cuda.get_device(t):
        ...
        softlabel = t[:,1:]
    soft_loss = - (F.sum(softlabel * F.log_softmax(x))) / x.shape[0]
    return soft_loss

def accfun(x, t):
    with cuda.get_device(t):
        label = t[:,0].astype('int32')
    return F.accuracy(x, label)
...
train_y = np.concatenate([train_y.reshape(-1, 1), train_soft], axis=1).astype(np.float32)
test_y = np.concatenate([test_y.reshape(-1, 1), test_soft], axis=1).astype(np.float32)
...
result_dir = '../results/softtopic_{}_100_depth{}_valid{}'.format(mode, sum(depth)*2+1, args.valid)
